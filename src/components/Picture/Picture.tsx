...
export const Picture: Picture = React.forwardRef((props, ref) => {
  ...
  if (mode === 2) {
    const images = image as { [key: string]: { [key: string]: string } | string };
    ...
    return (
      <picture ref={ref} className={className}>
        {keys.map((key) => {
          ...
          if (typeof src === 'string') {
            return <source key={key} media={`(min-width: ${key}px)`} src={src} />;
          }
          ...
          return <source key={key} media={`(min-width: ${key}px)`} srcSet={srcSet} />;
        })}
